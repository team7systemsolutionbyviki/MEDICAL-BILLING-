...
let branches = JSON.parse(localStorage.getItem('mediflow_branches')) || [{ id: 'main', name: 'Main Branch' }];
...
function checkLoginStatus() {
    ...
    if (isLoggedIn === 'true') {
        ...
        if (loggedInUsername === 'VIKI') {
            ...
            if (branchAdminSection) branchAdminSection.style.display = 'block';
            const unitsSection = document.getElementById('superadmin-units-section');
        if (unitsSection) unitsSection.style.display = actualRole === 'superadmin' ? 'block' : 'none';
        
        const amcSection = document.getElementById('superadmin-amc-section');
            if (amcSection) amcSection.style.display = 'block';
...
function renderSalesHistory() {
    try {
        ...
        let filteredSales = sales.filter(s => currentBranch === 'all' || !s.branchId || s.branchId === currentBranch);
...
function renderDashboard() {
    try {
        ...
        const branchSales = sales.filter(s => currentBranch === 'all' || !s.branchId || s.branchId === currentBranch);
        const branchExpenses = expenses.filter(e => currentBranch === 'all' || !e.branchId || e.branchId === currentBranch);
        const branchPurchases = purchases.filter(p => currentBranch === 'all' || !p.branchId || p.branchId === currentBranch);
        const branchProducts = products.filter(p => currentBranch === 'all' || !p.branchId || p.branchId === currentBranch);
...
function exportDataSilent(dateStr) {
    ...
    setTimeout(() => {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `MediFlow_DailyBackup_${dateStr}.json`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
